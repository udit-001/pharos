...
- **`mermaid-theme.js` is `//go:embed`'d in `internal/vendor/`** (global vendor cache; served at `/vendor/...`), **`style.css` in `internal/db/seed.go`** (seeded per workspace). Changing them requires `pharos build`; vendored lib updates mean editing `internal/vendor/manifest.go` (version + sha256) and running `pharos vendor sync`.
