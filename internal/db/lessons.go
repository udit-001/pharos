...
const lessonColumns = `id, workspace_id, title, sequence_number, filename, path, summary, created_at, updated_at`

func scanLesson(row interface{ Scan(...any) error }) (Lesson, error) {
	var l Lesson
	err := row.Scan(&l.ID, &l.WorkspaceID, &l.Title, &l.SequenceNumber, &l.Filename, &l.Path, &l.Summary, &l.CreatedAt, &l.UpdatedAt)
	return l, err
}
